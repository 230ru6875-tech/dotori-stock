  ...
  if (state.activeSection === "watchlist") {
    grid.innerHTML = renderCards(active, (item) => {
      ...
      if (item.tossHolding) {
        ...
        const holdingPrice = item.currentPrice ? `<span class="holding-current-price">현재가 ${formatDisplayPrice(item.currentPrice, item)}</span>` : "";
        return `<article class="data-card clickable-card watch-card ${priceBackgroundClassForItem(item)}" data-chart-symbol="${item.symbol}"><div class="card-top"><strong>${displayName}</strong>${holdingPrice}<em>${displayMarket(item.market)}</em></div>${purchaseLine}${item.memo ? `<p>${escapeHtml(item.memo)}</p>` : ""}<div class="watch-chart-popover">${watchlistMiniChartSvg(item)}</div></article>`;
      }
